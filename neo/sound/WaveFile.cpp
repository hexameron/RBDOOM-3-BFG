...
bool idWaveFile::Open( const char* filename )
{
	...
	if( header.id != WAVEIDRIFF || header.format != WAVEIDWAVE || header.size < 4 )
	{
		Close();
...
{
	static const uint32 sample = WAVEIDsmpl;
...
{
	static const uint32 data = WAVEIDdata;
	file->WriteBig( data );
...
{
	static const uint32 riff = WAVEIDRIFF;
	static const uint32 wave = WAVEIDWAVE;
	file->WriteBig( riff );
